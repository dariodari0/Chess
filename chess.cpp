...
public:
    Bishop(bool _white):white(_white){}
    bool valid(const Move &m, vector<Coord>& list, bool errorMessagesOn = true)
    {
...
public:
    Queen(bool _white):Bishop(_white){}
...
bool Board::valid(const Move& m, bool errorMessagesOn, bool isDeadlockMode) {
    ...
    //TODO przerobic Figure board[64] na Figure * board[64]
    ...
    if (!(board(m.from.row, m.from.col).valid(m, list, errorMessagesOn))) {
        return false;
    }
